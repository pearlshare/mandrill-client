...
module.exports = function mandrillClient(config) {
  ...
  function sendMessage (message, opts) {
    var sendUrl = "messages/send.json";
    ...
    // Template support
    if (opts.template_name !== undefined && opts.template_content !== undefined) {
      /*eslint-disable camelcase*/
      body.template_name = opts.template_name;
      body.template_content = opts.template_content;
      /*eslint-enable */

      sendUrl = "messages/send-template.json"; // Change URL to support templates
    }

    return makeRequest(sendUrl, body);
  }
